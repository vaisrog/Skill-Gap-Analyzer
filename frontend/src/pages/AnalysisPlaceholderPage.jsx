...
import { AlertCircle, BarChart3, CheckCircle2, RefreshCw, Sparkles, Target } from 'lucide-react';
...
export const AnalysisPlaceholderPage = () => {
  ...
  return <div className="min-h-screen flex flex-col bg-slate-50"><Navbar /><div className="flex-1 flex max-w-7xl w-full mx-auto"><Sidebar /><main className="flex-1 p-6 space-y-6 overflow-y-auto">
    ...
    {recommendation ? <section className="bg-blue-50 border border-blue-200 rounded-2xl p-5"><div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shrink-0"><Sparkles className="w-5 h-5" /></div><div className="flex-1"><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Recommended Next Skill</p><div className="mt-1 flex flex-wrap items-center gap-2"><h2 className="text-lg font-extrabold text-slate-900">{recommendation.skill_name}</h2><SkillPill label={recommendation.importance} style={importanceStyles[recommendation.importance]} /><SkillPill label={recommendation.status} style={statusStyles[recommendation.status]} /></div><p className="mt-2 text-sm text-slate-700 leading-relaxed">{recommendation.recommendation_reason}</p><p className="mt-2 text-xs font-semibold text-slate-600">Current: {recommendation.current_proficiency_label} · Required: {recommendation.required_proficiency_label} · Gap: {recommendation.skill_gap}</p></div></div></section> : <section className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex gap-3"><CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" /><div><p className="font-bold text-emerald-900">All listed requirements are satisfied.</p><p className="text-sm text-emerald-800 mt-1">You meet or exceed every configured skill requirement for this career.</p></div></section>}
